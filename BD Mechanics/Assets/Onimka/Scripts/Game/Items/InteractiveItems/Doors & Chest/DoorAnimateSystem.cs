...
public class DoorAnimateSystem : MonoBehaviour
{
    ...
    [SerializeField] private DispleyVisible _itemInfo;
        ...
        if (Open)
        {
            switch (axisRotation)
            {
                case AxisRotation.rotX:
                    _door.Rotate(animationSettings.openAngle, 0, 0); 
                    break;
                case AxisRotation.rotY:
                    _door.Rotate(0, animationSettings.openAngle, 0);
                    break;
                case AxisRotation.rotZ:
                    _door.Rotate(0, 0, animationSettings.openAngle);
                    break;
            }
        }

        if (_navObstacle != null)
            _navObstacle.enabled = !Open;
    }
    private bool HasKey()
    {
    ...
    IEnumerator OpenDoorCor()
    {
        while (isOpening)
        {
            ...
            {
                case AxisRotation.rotX:
                    _door.rotation = Quaternion.Lerp(initialRotation, initialRotation * Quaternion.Euler(animationSettings.openAngle, 0, 0), curveValue);
                    break;
                case AxisRotation.rotY:
                    _door.rotation = Quaternion.Lerp(initialRotation, initialRotation * Quaternion.Euler(0, animationSettings.openAngle, 0), curveValue);
                    break;
                case AxisRotation.rotZ:
                    _door.rotation = Quaternion.Lerp(initialRotation, initialRotation * Quaternion.Euler(0, 0, animationSettings.openAngle), curveValue);
                    break;
            }
    ...
    IEnumerator CloseDoorCor()
    {
        while (isClosing)
        {
            ...
            {
                case AxisRotation.rotX:
                    _door.rotation = Quaternion.Lerp(initialRotation * Quaternion.Euler(animationSettings.openAngle, 0, 0), initialRotation, curveValue);
                    break;
                case AxisRotation.rotY:
                    _door.rotation = Quaternion.Lerp(initialRotation * Quaternion.Euler(0, animationSettings.openAngle, 0), initialRotation, curveValue);
                    break;
                case AxisRotation.rotZ:
                    _door.rotation = Quaternion.Lerp(initialRotation * Quaternion.Euler(0, 0, animationSettings.openAngle), initialRotation, curveValue);
                    break;
            }
    ...
    {
        Quaternion initialRotation = _door.rotation;
        ...
        while (timer < ShakeDuration)
        {
            ...
            _door.rotation = initialRotation * Quaternion.Euler(shakeX, shakeY, 0f);
            yield return null;
        }

        _door.rotation = initialRotation; 
        audioSettings.SoundStop();
    }
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        {
            if (_itemInfo.Selected)
            {
                _itemInfo.Selected = false;
                Interaction();               
            }
        }
    }
